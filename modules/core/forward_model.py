...
#def process_buoy(scene_id, buoy_id, data, caller, status_logger, overpass_date, atmo_source, verbose, bands, rsrs, directory, metadata):
#    
...
#    image, overpass_date, directory, metadata, file_downloaded = display.landsat_preview(scene_id, '')
    image_data = display.landsat_preview(scene_id, '')
...
#        display_image_thread.start()
    ...
    cv2.imwrite(absolute_image_path, image_data['image'])
        
    data = {}
    
    if image_data['file_downloaded']:
        # satelite download
...
#        overpass_date, directory, metadata, file_downloaded = sat.landsat.download(scene_id, bands[:])
        ...
        corners = sat.landsat.corners(image_data['metadata'])
        buoys = buoy.datasets_in_corners(corners)
...
#            buoy_thread = threading.Thread(target=process_buoy, args=(scene_id, buoy_id, data, caller, status_logger, overpass_date, atmo_source, verbose, bands, rsrs, directory, metadata, ))
#            buoy_threads.append(buoy_thread)
#            
#        
...
#            }
            ...
            try:
                buoy_file = buoy.download(buoy_id, image_data['overpass_date']) ##To this point is fine - to use method in new class when completed - create new buoy object as this point
#                buoy_lat, buoy_lon, buoy_depth, bulk_temp, skin_temp, lower_atmo = buoy.info(buoy_id, buoy_file, image_data['overpass_date'])
                buoy_data = buoy.info(buoy_id, buoy_file, image_data['overpass_date'])
...
### Continue from here
            ...
            # Atmosphere
            if atmo_source == 'merra':
                atmosphere = atmo.merra.process(image_data['overpass_date'], buoy_data['buoy_lat'], buoy_data['buoy_lon'], verbose)
            elif atmo_source == 'narr':
                atmosphere = atmo.narr.process(image_data['overpass_date'], buoy_data['buoy_lat'], buoy_data['buoy_lon'], verbose)
            else:
                raise ValueError('atmo_source is not one of (narr, merra)')
...
#                wavelengths, upwell_rad, gnd_reflect, transmission = modtran.process(atmosphere, buoy_data['buoy_lat'], buoy_data['buoy_lon'], image_data['overpass_date'], modtran_directory, buoy_data['skin_temp'])
                modtran_data = modtran.process(atmosphere, buoy_data['buoy_lat'], buoy_data['buoy_lon'], image_data['overpass_date'], modtran_directory, buoy_data['skin_temp'])
                
                # LTOA calcs
#                mod_ltoa_spectral = radiance.calc_ltoa_spectral(wavelengths, upwell_rad, gnd_reflect, transmission, buoy_data['skin_temp'])
                mod_ltoa_spectral = radiance.calc_ltoa_spectral(modtran_data['wavelengths'], modtran_data['upwell_rad'], modtran_data['gnd_reflect'], modtran_data['transmission'], buoy_data['skin_temp'])
                ...
                try:
                    for b in bands:
                        RSR_wavelengths, RSR = numpy.loadtxt(rsrs[b], unpack=True)
                        img_ltoa[b] = sat.landsat.calc_ltoa(image_data['directory'], image_data['metadata'], buoy_data['buoy_lat'], buoy_data['buoy_lon'], b)
#                        mod_ltoa[b] = radiance.calc_ltoa(wavelengths, mod_ltoa_spectral, RSR_wavelengths, RSR)
                ...
                error = error_bar.error_bar(scene_id, buoy_id, buoy_data['skin_temp'], 0.305, image_data['overpass_date'], buoy_data['buoy_lat'], buoy_data['buoy_lon'], rsrs, bands)
